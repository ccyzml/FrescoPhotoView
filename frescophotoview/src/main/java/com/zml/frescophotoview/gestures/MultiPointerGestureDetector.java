...
public class MultiPointerGestureDetector {
    ...
    private final float mLastX[] = new float[MAX_POINTERS];
    private final float mLastY[] = new float[MAX_POINTERS];
    ...
    private void updatePointersOnTap(MotionEvent event) {
        ...
        for (int i = 0; i < MAX_POINTERS; i++) {
            ...
            if (index == -1) {
                ...
                mCurrentX[i] = mStartX[i] = mLastX[i] = event.getX(index);
                mCurrentY[i] = mStartY[i] = mLastY[i] = event.getY(index);
                mPointerCount++;
            }
        }
    }
    ...
    private void updatePointersOnMove(MotionEvent event) {
        for (int i = 0; i < MAX_POINTERS; i++) {
            ...
            if (index != -1) {
                mLastX[i] = mCurrentX[i];
                mLastY[i] = mCurrentY[i];
