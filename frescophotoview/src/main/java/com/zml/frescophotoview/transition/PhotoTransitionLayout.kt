 ...
 */
open class PhotoTransitionLayout : FrameLayout, TransformGestureDetector.Listener,
    DragTransitionView {
    ...
    private val detector: TransformGestureDetector = TransformGestureDetector.newInstance()
    private var transitionState = -1
    private var skipLayout = false
    private var dragTransitionViewDelegate: DragTransitionView? = null
    ...
    init {
        ...
        detector.ignoreGestureIntent(TransformGestureDetector.GESTURE_INTENT_DIRECTION_UP)
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent?): Boolean {
        if (dragTransitionViewDelegate == null) {
            detector.onTouchEvent(ev)
            requestDisallowInterceptTouchEvent(transitionState == DragTransitionView.STATE_DRAG_TRANSITION)
        }
        return transitionState == DragTransitionView.STATE_DRAG_TRANSITION
    }

    override fun onTouchEvent(event: MotionEvent?): Boolean {
        if (transitionState == DragTransitionView.STATE_DRAG_TRANSITION) {
    ...
    override fun onGestureBegin(detector: TransformGestureDetector?) {
        transitionState = DragTransitionView.STATE_UNDEFINE
    }

    override fun onGestureUpdate(detector: TransformGestureDetector?) {
        if (this.detector.gestureIntent == TransformGestureDetector.GESTURE_INTENT_DIRECTION_DOWN) {
            transitionState = DragTransitionView.STATE_DRAG_TRANSITION
            transitionListener?.onTransitionBegin(transitionState)
        }
        if (transitionState == DragTransitionView.STATE_DRAG_TRANSITION) {
    ...
    override fun onGestureEnd(detector: TransformGestureDetector?) {
        if (transitionState == DragTransitionView.STATE_DRAG_TRANSITION) {
            ...
            if (dragFactor < eps) {
                ...
                transitionState = DragTransitionView.STATE_OUT_TRANSITION
                animator.start()
                alphaAnimator.start()
            } else {
                startRect.set(workingRect)
                transitionState = DragTransitionView.STATE_RESUME_TRANSITION
                animator.start()
            }
        }
    }
    ...
    fun startEnterTransition(@NonNull startRect: Rect) {
        ...
        transitionState = DragTransitionView.STATE_ENTER_TRANSITION
        this.startRect = startRect
        animator.start()
        postDelayed({ skipLayout = false }, 50)
    }

    fun setDelegateDragTransitionView(dragTransitionView: DragTransitionView) {
        dragTransitionViewDelegate = dragTransitionView
        dragTransitionViewDelegate?.setTransitionListener(transitionListener)
    }
}
